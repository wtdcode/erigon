...
func (ac *AggregatorV3Context) PruneSmallBatches(ctx context.Context, timeout time.Duration, tx kv.RwTx) error {
	...
	fullStat := &AggregatorPruneStat{Indices: make(map[string]*InvertedIndexPruneStat)}
...
type AggregatorPruneStat struct {
	Domains [kv.DomainLen]*DomainPruneStat
...
func (ac *AggregatorV3Context) Prune(ctx context.Context, tx kv.RwTx, limit uint64, logEvery *time.Ticker) (*AggregatorPruneStat, error) {
	...
	//
	aggStat := &AggregatorPruneStat{Indices: make(map[string]*InvertedIndexPruneStat)}
	for id := range ac.d {
		var err error
		aggStat.Domains[id], err = ac.d[id].Prune(ctx, tx, step, txFrom, txTo, limit, logEvery)
		if err != nil {
			return nil, err
		}
	}
