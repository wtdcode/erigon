...
func (r *BlockReader) EventsByBlock(ctx context.Context, tx kv.Tx, hash common.Hash, blockHeight uint64) ([]rlp.RawValue, error) {
	...
	log.Warn("[dbg] EventsByBlock0", "maxBlockNumInFiles", maxBlockNumInFiles, "blockHeight", blockHeight)
	if maxBlockNumInFiles == 0 || blockHeight > maxBlockNumInFiles {
		...
		if !bytes.Equal(k, buf[:]) {
			log.Warn("[dbg] BorEventsByBlock1", "block", blockHeight)
			return result, nil
		}
		...
		log.Warn("[dbg] BorEventsByBlock10", "endEventId", endEventId)
		...
		for k, v, err = c1.Seek(buf[:]); k != nil; k, v, err = c1.Next() {
			if err != nil {
				return nil, err
			}
